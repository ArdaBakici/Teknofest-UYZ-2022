...
weight_loc = './weights/uap_v_latest.pt' # Modelin ağırlık dosyasının konumu
...
def find(img0, save_loc=None, _augment=False, conf_thres=0.4, iou_thres=0.5, agnostic_nms=False):
    with torch.no_grad():
        ...
        for i, det in enumerate(pred):  # detections per image
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    x1 = int(xyxy[2])
                    y1 = int(xyxy[3])
                    predictions.append([int(cls)+2, x0, y0, x1, y1])
